    ...
    <table class="table" id="lineUps-table">
        ...
        <tbody>
            @foreach($lineUps as $lineUp)
                ...
                <td>{{ $lineUp->img }}</td>
                <td>
                   <form action="{{ route('lineup.activar', $lineUp) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="custom-control custom-switch">
                            <input
                                ...
                                id="activar-{{ $lineUp->id }}"
                                name="activa"
                                value="1"
                                onchange="this.form.submit()"
                                {{ $lineUp->activa ? 'checked' : '' }}
                            >
                            <label class="custom-control-label" for="activar-{{ $lineUp->id }}"></label>
                        </div>
                    </form>
